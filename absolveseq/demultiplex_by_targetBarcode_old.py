...
OLIGO_POOL = pd.read_excel("./test/data/target_meta/LVOT_oligo_pool.xlsx", engine = "openpyxl")
OLIGO_POOL = OLIGO_POOL.set_index("Barcode")["OT"].to_dict()

def put_umi_in_header(r1, out_dir, constant_seq="CCAACCTCATAGAACA"):
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='.', help='Directory to save output FASTQ files and summaries.')
    parser.add_argument('--n_cpu', type=int, default=10, help='Number of CPU cores to use for parallel processing.')
    ...
    # process each sample: pass (r1_path, out_dir) to worker
    dict_list, info_list, UMIs_per_OT_list = zip(
        *pool.starmap(put_umi_in_header, [(r1, out_dir) for r1 in fastq_files])
    )
    ...
    os.makedirs(out_dir + "/plasmid_barcodes/", exist_ok=True)
    ...
    for i in range(len(info_list)):
        ...
        df.columns = ["Reads"]
        df.to_csv(out_dir + "/plasmid_barcodes/" + sample + ".csv")
    ...
    summary_df = pd.DataFrame(
        info_list,
        columns=["Sample", "Total reads", "Reads with oligo barcode", "Corresponding UMIs"]
    )
